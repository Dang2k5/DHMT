...
void drawRobotStore(vec3 position, vec3 rotation, vec3 scale)
{
    ...
    // nền
    drawCylinder(vec3(3, -48, 10), vec3(0, 0, 0), vec3(20, .1, 15), color2);


    drawCube(vec3(0, -50, 0), vec3(0, 0, 0), vec3(100, .5, 45), color4); // sàn nhà
    drawCube(vec3(0, -52, 0), vec3(0, 0, 0), vec3(100, 4, 50), frameColor);
    drawCube(vec3(0, 40.5, 0), vec3(0, 0, 0), vec3(72, 1.5, 52), frameColor);// nóc nhà
   ...
   /* drawCube(vec3(-8, 20, 23), vec3(0, 0, 0), vec3(2, 40, 2), color1);
    ...
    //bac thang
    drawCube(vec3(3, -50, 26), vec3(0, 0, 0), vec3(20, 2, 2), frameColor1);
    drawCube(vec3(3, -50.5, 27), vec3(0, 0, 0), vec3(20, 2, 5), frameColor1);

    //duong
    /*drawCube(vec3(0, -2, 0), vec3(0, 0, 0), vec3(100, 1, 100), frameColor);*/

    //kinh
    drawCube(vec3(23, 15, 23), vec3(0, 0, 45), vec3(10, 2, .01), color(1,0,0,1));
