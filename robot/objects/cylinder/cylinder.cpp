...
void createCylinder() {
	float angelStep = 2.0f * M_PI / CYLINDER_SIDES;
...
void cylinder_quad(int a, int b, int c, int d) {
	...
	cylinder_normals[cylinder_index] = normal; cylinder_points[cylinder_index] = cylinder_vertices[a]; cylinder_index++;
	cylinder_normals[cylinder_index] = normal; cylinder_points[cylinder_index] = cylinder_vertices[b]; cylinder_index++;
	cylinder_normals[cylinder_index] = normal; cylinder_points[cylinder_index] = cylinder_vertices[c]; cylinder_index++;
	cylinder_normals[cylinder_index] = normal; cylinder_points[cylinder_index] = cylinder_vertices[a]; cylinder_index++;
	cylinder_normals[cylinder_index] = normal; cylinder_points[cylinder_index] = cylinder_vertices[c]; cylinder_index++;
	cylinder_normals[cylinder_index] = normal; cylinder_points[cylinder_index] = cylinder_vertices[d]; cylinder_index++;
}
...
void makeCylinder() {
    ...
    for (int i = 0; i < CYLINDER_SIDES; i++) {
        int next = (i + 1) % CYLINDER_SIDES;
        cylinder_normals[cylinder_index] = topNormal; cylinder_points[cylinder_index] = topCenter; cylinder_index++;
        cylinder_normals[cylinder_index] = topNormal; cylinder_points[cylinder_index] = cylinder_vertices[i]; cylinder_index++;
        cylinder_normals[cylinder_index] = topNormal; cylinder_points[cylinder_index] = cylinder_vertices[next]; cylinder_index++;
    }
    vec3 bottomNormal = vec3(0.0f, -1.0f, 0.0f);
    point4 bottomCenter = point4(0.0f, -0.5f, 0.0f, 1.0f);
    for (int i = 0; i < CYLINDER_SIDES; i++) {
        int next = (i + 1) % CYLINDER_SIDES;
        cylinder_normals[cylinder_index] = bottomNormal; cylinder_points[cylinder_index] = bottomCenter; cylinder_index++;
        cylinder_normals[cylinder_index] = bottomNormal; cylinder_points[cylinder_index] = cylinder_vertices[next + CYLINDER_SIDES]; cylinder_index++;
        cylinder_normals[cylinder_index] = bottomNormal; cylinder_points[cylinder_index] = cylinder_vertices[i + CYLINDER_SIDES]; cylinder_index++;
    }
}
...
void drawCylinder(const vec3& position, const vec3& rotation, const vec3& scale, const color& mainColor, engine::shader shader)
{
    ...
    glUniformMatrix4fv(shader.model_loc, 1, GL_TRUE, cylinderModelMatrix * instance);
